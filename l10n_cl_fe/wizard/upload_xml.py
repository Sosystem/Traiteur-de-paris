...
class UploadXMLWizard(models.TransientModel):
    ...
    def _get_invoice_lines(self, documento, document_id, invoice_type, fpos, price_included, company_id, document=False):
        ...
        for line in documento.findall("Detalle"):
            if not self.pre_process and self.option == "upload" and self.type == 'compras' and documento.find("Encabezado/Totales/MontoNF") is not None and line.find("IndExe") is not None:
                continue
